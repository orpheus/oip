...
func init() {
	log.Info("init tZero")
	if !config.IsTestnet() {
		events.SubscribeAsync("flod:floData", floDataProcessor)
		events.SubscribeAsync("specs:tZero:cancel", onCancel)
		events.SubscribeAsync("specs:tZero:inventoryPosted", onInventoryPosted)
		events.SubscribeAsync("specs:tZero:executionReport", onExecutionReport)
		events.SubscribeAsync("specs:tZero:clientInterest", onClientInterest)
...
func floDataProcessor(floData string, tx *datastore.TransactionData) {
	...
	if strings.HasPrefix(floData, "Cancel: ") {
		events.Publish("specs:tZero:cancel", floData, tx)
		return
	}
	if strings.HasPrefix(floData, "Inventory Posted: ") {
		events.Publish("specs:tZero:inventoryPosted", floData, tx)
		return
	}
	if strings.HasPrefix(floData, "Execution Report: ") {
		events.Publish("specs:tZero:executionReport", floData, tx)
		return
	}
	if strings.HasPrefix(floData, "Client Interest: ") {
		events.Publish("specs:tZero:clientInterest", floData, tx)
		return
	}
}
