...
func init() {
	log.Info("init oip")
	if config.IsTestnet() {
		events.SubscribeAsync("flod:floData", onFloDataTestNet)
	} else {
		events.SubscribeAsync("flod:floData", onFloDataMainNet)
	}
...
func onFloDataMainNet(floData string, tx *datastore.TransactionData) {
	...
	if tx.Block < 2731000 && tx.Transaction.Vin[0].IsCoinBase() {
		...
		if strings.HasPrefix(simplified, "oip-historian-") ||
			...
			events.Publish("specs:historian:stringDataPoint", floData, tx)
		}
	}
	...
	if (tx.Block > 2263000 && strings.HasPrefix(simplified, "oip-mp(")) ||
		...
		events.Publish("specs:oip:multipartSingle", floData, tx)
		return
	}

	if strings.HasPrefix(simplified, `{"alexandria-publisher":`) {
		events.Publish("specs:oip:alexandriaPublisher", floData, tx)
		return
	}

	if tx.Block < 2400000 {
		if strings.HasPrefix(simplified, `{"alexandria-deactivation":`) {
			events.Publish("specs:oip:alexandriaDeactivation", floData, tx)
			return
		}
		if strings.HasPrefix(simplified, `{"alexandria-media":`) {
			events.Publish("specs:oip:alexandriaMedia", floData, tx)
			return
		}
	}
	...
	if strings.HasPrefix(simplified, `{"oip-041":`) {
		events.Publish("specs:oip:oip041", floData, tx)
		return
	}
	...
	if processPrefix("text:", "flod:floData", floData, tx) {
		return
	}
}
...
func onJson(floData string, tx *datastore.TransactionData) {
	...
	if o42, ok := dj["oip042"]; ok {
		...
		events.Publish("specs:oip042:json", o42, tx)
		return
	}
...
func processProto(b []byte, tx *datastore.TransactionData, attr logger.Attrs) {
	...
	case pb_oip.SignatureTypes_Flo:
		...
		if err != nil || !valid {
			...
			attr["sigType"] = msg.SignatureType
			attr["signature"] = signature
			log.Error("flod signature validation failed", attr)
			return
		}
	default:
		attr["sigType"] = msg.SignatureType
	...
	case pb_oip.MessageTypes_Historian:
		events.Publish("specs:historian:protoDataPoint", msg, tx)
	case pb_oip.MessageTypes_OIP05:
		events.Publish("specs:oip5:msg", msg, tx)
	case pb_oip.MessageTypes_Multipart:
		events.Publish("specs:oip:multipartProto", msg, tx)
	default:
		attr["err"] = err
